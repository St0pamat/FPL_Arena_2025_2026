...
const TrackCard = ({
  ...
  const downloadLabel = soundtrackWavDownloadName(order, playerDisplayName(player));
  ...
  const downloadItems = useMemo(
    () =>
      tracks.map(({ order, player, file }) => ({
        url: soundtrackFileUrl(file),
        filename: soundtrackWavDownloadName(order, playerDisplayName(player)),
      })),
    [tracks]
  );

  const handleDownloadAll = async () => {
    if (batchBusy || downloadItems.length === 0) return;
    setBatchBusy(true);
    setBatchProgress(0);
    try {
      await downloadAllSoundtracks(downloadItems, (current) => setBatchProgress(current));
    } finally {
      setBatchBusy(false);
    }
  };
      ...
      <div className="glass-panel panel-pad rounded-2xl border border-violet-500/25 bg-gradient-to-br from-violet-500/10 via-slate-900/40 to-emerald-500/5 flex flex-col md:flex-row md:items-center gap-6">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-fluid-sm text-slate-400 mt-2 leading-relaxed">
            Pobierz wszystkie zapowiedzi Gladiatorów naraz (20 plików WAV). Przeglądarka może poprosić o
            zgodę na wiele pobrań — zaakceptuj, aby zapisac cala playliste.
          </p>
        </div>
        <div className="shrink-0 flex flex-col items-stretch sm:items-end gap-2">
          <button
            ...
            disabled={batchBusy}
            className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-fluid-sm font-bold border-2 border-emerald-500/50 bg-emerald-500/20 text-emerald-200 hover:bg-emerald-500/30 hover:border-emerald-400 transition-all shadow-[0_0_24px_rgba(16,185,129,0.15)] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <span aria-hidden>{batchBusy ? "…" : "⬇"}</span>
            {batchBusy ? `Pobieranie… ${batchProgress}/${tracks.length}` : "Pobierz wszystkie (20 WAV)"}
          </button>
          {batchBusy && (
            <p className="text-fluid-xs text-slate-500 text-center sm:text-right">
              Uruchamianie kolejnych plikow…
            </p>
          )}
        </div>
      </div>
